...
<script>
import { getAddressList, checkOrder, submitOrder } from '@/api/payApi'
...
export default {
  ...
  computed: {
    ...
    longAddress () {
      const regions = this.selAddress.region
      return regions.province + regions.city + regions.region + this.selAddress.detail
    },
    mode () {
...
.address {
  ...
  color: #666;
  position: relative;
  background: url(@/assets/pic/border-line.png) bottom repeat-x;
