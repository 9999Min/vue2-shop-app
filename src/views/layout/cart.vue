<template>
  <div class="cart">
    ...
    <div class="cart-box" v-if="isLogin && cartList.length > 0">
      ...
      <div class="cart-item" v-for="item in cartList" :key="item.goods_id">
        ...
        <div class="info">
          ...
          <span class="bottom">
            ...
            <!-- 既保留原本的形参，又希望通过方法调用传参 => 使用箭头函数包一层 -->
      ...
      <div class="all-check">
        <van-checkbox @click="toggleAllCheck(isAllChecked)" icon-size="18" :value="isAllChecked"></van-checkbox>
        全选
      </div>
      ...
      <div class="all-total">
        ...
        <div v-if="edit" class="delete" :class="{disable: selCount === 0 }" @click="handleDelete">删除</div>
        <div v-else class="goPay" :class="{disable: selCount === 0}" @click="gopay">结算({{selCount}})</div>
      </div>
      </div>
    </div>
    <div class="empty-cart" v-else>
      <img src="@/assets/pic/empty.png" alt="">
      <div class="tips">
        您的购物车是空的, 快去逛逛吧
      </div>
...
export default {
  ...
  methods: {
    ...
    toggleAllCheck (status) {
      this.$store.commit('cart/toggleAllCheck', !status)
    },
    ...
    async handleDelete () {
      ...
      await this.$store.dispatch('cart/deleGoods')
      this.edit = false
    },
    gopay () {
      if (this.selCount > 0) {
